...
#*Função split_series*: Entre as séries importadas da FS, a função retorna apenas uma série empilhada selecionada. O argumento type retorna a série *realizado*, *projetado* ou *ambos* 
...
```{r}
model_snaive = snaive(df, nmeans = 1, end_projection = "2026-12-01")
# nmeans 
...
ggplot(data = dessaz_stl)+
       aes(x = original.date)+
       geom_line(aes(y = original.vl, col = "Original"))+
       geom_line(aes(y = dessaz.stl, col = "Dessaz STL"))+
       labs(x = "", y = "value")+
       theme_bw()+
       theme(legend.position = "bottom")
...
ggplot(data = dessaz)+
       ...
       theme(legend.position = "bottom")
```
#*Função get_seas_adj*: Calcula o dessaz de uma série com as configurações automáticas do STL ou do X13, opcional retornar a média dos dois. O argumento *type*  representa o dessaz *STL*, *X13* ou agregações como *mean* ou *median* (**O default é 'median'**)

```{r}
dessaz_adj_mediana = get_seas_adj(df, type = "median")
