...
#' @name get_realvl
#'
#' @description A função calcula o a deflação de uma série nominal.
#'
...
#'
#' @examples
#' get_realvl(price_fuel, #Preço gasolina
#'            ipca_br,
#'            '2022-12-01'
#'            )
#'
...
get_realvl <- function(df, df_cpi, dt_base_in, dt_base_fim = NULL) {
  ...
  real <- df %>%
    dplyr::rename(nominal = vl) %>%
    dplyr::left_join(df_cpi %>%
                       dplyr::rename(cpi = vl)) %>%
    dplyr::mutate(real = nominal * (cpi_vl / cpi))
